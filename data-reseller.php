...
if (isset($_GET['logout'])) {
    session_destroy();
    header("Location: data-reseller.php");
    exit;
}
...
$step  = $_SESSION['login_reseller'] ?? 'pilih_nama';
$error = "";
...
if (isset($_POST['pilih_nama'])) {
    ...
    unset($_SESSION['login_reseller']); // reset login
    $step = "password";
}
...
if (isset($_POST['cek_password'])) {
    ...
    if ($password === $password_nama[$nama_pilih]) {
        $_SESSION['login_reseller'] = 'tampil_data';
        $step = "tampil_data";
    } else {
        $step  = "password";
...
if (
    ...
    SELECT SUM(fee_rupiah_reseller) total
    FROM data_orderan_reseller
    WHERE nama='$nama'
    AND YEAR(tanggal)='$tahun'
    AND MONTH(tanggal)='$bulan'
    AND tagihan_reseller >= harga_barang
    "))['total'] ?? 0;
    ...
    SELECT SUM(harga_barang) total
    FROM data_orderan_reseller
    WHERE nama='$nama'
    AND YEAR(tanggal)='$tahun'
    AND MONTH(tanggal)='$bulan'"))['total'] ?? 0;

    $totalBuyTahun = mysqli_fetch_assoc(mysqli_query($conn, "
    SELECT SUM(harga_barang) total
    FROM data_orderan_reseller
    WHERE nama='$nama'
    AND YEAR(tanggal)='$tahun'"))['total'] ?? 0;



    $totalPiutang = mysqli_fetch_assoc(mysqli_query($conn, "
    SELECT SUM(harga_barang - tagihan_reseller) AS total
    FROM data_orderan_reseller
    WHERE nama = '$nama'
    AND harga_barang > tagihan_reseller
    "))['total'] ?? 0;
    ...
    $q = mysqli_query($conn, "
    SELECT MONTH(tanggal) bulan, SUM(fee_rupiah_reseller) total
    FROM data_orderan_reseller
    WHERE nama='$nama'
    AND YEAR(tanggal)='$tahun'
    AND tagihan_reseller >= harga_barang
    ...
    $data = mysqli_query($conn, "
        SELECT * FROM data_orderan_reseller
        WHERE nama='$nama'
        AND YEAR(tanggal)='$tahun'
...
<head>
    <title>Data Reseller - Saung-Syar'i</title>
...
<body>
    <div class="app">
        ...
        <aside class="sidebar">
            ...
            <nav>
                ...
                <a href="data-reseller.php" class="active">🛒 Reseller</a>
                ...
                <div class="card">
                    ...
                    <form method="POST">
                        <select name="nama" required>
                            ...
                            <option value="UMI F">UMI FADILLAH</option>
                            <option value="IIS H">IIS HERLIANA</option>
                            <option value="SRI">SRI ARYANTI</option>
                            <option value="AINUN">AINUN ALFI</option>
                            <option value="FERA F">FERA FERDIANA</option>
                            <option value="IIS SITI">IIS SITI NUR A</option>
    ...
    margin:20px 0;
">
                    <h4>Total Tagihan Reseller</h4>
                    <h2>Rp <?= number_format($totalPiutang); ?></h2>
                </div>
